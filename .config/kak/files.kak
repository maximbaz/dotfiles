...
define-command files-in-folder -docstring 'Open one or many files in a recent folder' %{ evaluate-commands %sh{
    FOLDER=$(cat "$HOME/.z" | sed 's/|/\t/g' | sort -rnk2 | awk '{print $1}' | sed "s|$HOME|~|g" | dmenu -p | sed "s|~|$HOME|g")
    if [ -n "$FOLDER" ]; then
        FILES=$(rg --hidden --files "$FOLDER" | sed "s|$FOLDER/||g" | dmenu -p)
        for file in $FILES; do
...
define-command buffers -docstring 'Switch to a buffer' %{ evaluate-commands %sh{
    BUFFER=$(eval set -- "$kak_buflist"; for buf in "$@"; do echo "$buf"; done | dmenu -p)
